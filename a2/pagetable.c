...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// Get the pte from memory
		pgtbl_entry_t *victim_page = coremap[frame].pte;

		// Check if the page needs to be written to swap
		// This happens under two conditions: The frame was changed (is now dirty)
		// OR the page has simply not yet been written to swap
		if ((victim_page->frame & PG_DIRTY) || !(victim_page->frame & PG_ONSWAP)) {

			evict_dirty_count++;

			// Change the PG_ONSWAP bit to high (indicate that the page is on swap)
			victim_page->frame |= PG_ONSWAP;

			// Actually write the page to swap
			swap_pageout(frame, victim_page->swap_off);
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// init second level if it's not inited before
	...
	unsigned int index = PGTBL_INDEX(vaddr);
	p = &(pgtbl[index]);

	// add one more ref_count
	ref_count++;
